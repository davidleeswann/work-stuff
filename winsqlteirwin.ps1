...
$fesubnet = New-AzVirtualNetworkSubnetConfig -Name 'Subnet-FrontEnd' -AddressPrefix '192.168.1.0/24'
$besubnet = New-AzVirtualNetworkSubnetConfig -Name 'Subnet-BackEnd' -AddressPrefix '192.168.2.0/24'
$vnet = New-AzVirtualNetwork -ResourceGroupName $rgName -Name 'Vnet' -AddressPrefix '192.168.0.0/16' `
...
Set-AzVirtualNetworkSubnetConfig -VirtualNetwork $vnet -Name 'Subnet-FrontEnd' `
  -AddressPrefix '192.168.1.0/24' -NetworkSecurityGroup $nsgfe
...
Set-AzVirtualNetworkSubnetConfig -VirtualNetwork $vnet -Name 'Subnet-BackEnd' `
  -AddressPrefix '192.168.2.0/24' -NetworkSecurityGroup $nsgbe

# Create a public IP address for the web server VM.
$publicipvm1 = New-AzPublicIpAddress -ResourceGroupName $rgName -Name 'PublicIp-Web' `
  -location $location -AllocationMethod Dynamic

# Create a NIC for the web server VM.
$nicVMweb = New-AzNetworkInterface -ResourceGroupName $rgName -Location $location `
  -Name 'Nic-Web' -PublicIpAddress $publicipvm1 -NetworkSecurityGroup $nsgfe -Subnet $vnet.Subnets[0]
...
$vmConfig = New-AzVMConfig -VMName 'MyVm-Web' -VMSize 'Standard_DS2' | `
  ...
  -Skus '2019-Datacenter' -Version latest | Add-AzVMNetworkInterface -Id $nicVMweb.Id
...
$vmConfig = New-AzVMConfig -VMName 'MyVm-Sql' -VMSize 'Standard_DS2' | `
  ...
  Set-AzVMSourceImage -PublisherName 'MicrosoftSQLServer' -Offer 'SQL2017-WS2016' `
